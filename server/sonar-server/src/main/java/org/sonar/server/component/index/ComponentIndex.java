...
public class ComponentIndex extends BaseIndex {
  ...
  private QueryBuilder createQuery(ComponentIndexQuery query) {
    ...
    return esQuery.must(boolQuery()
      .should(matchQuery(FIELD_NAME + "." + SEARCH_PARTIAL_SUFFIX, truncatedQuery)).boost(3f)
      .should(fuzzyQuery(FIELD_NAME + "." + SEARCH_FUZZY_SUFFIX, queryText.toLowerCase()))
      .should(matchQuery(FIELD_KEY + "." + SORT_SUFFIX, queryText).boost(10f)));
  }
